...
const sx = {
  ...
  logoText:  { fontSize:16, fontWeight:800, color:C.text, letterSpacing:-0.5 },
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
      <div style={sx.trow}>
        {PRESET_MANAGERS.map((m, i) => (
          <button key={m} style={{ ...sx.toggle(!customMode && value===m, i===0?'#0284C7':'#D97706'), padding:'9px 24px', fontSize:14, fontWeight:700 }}
            onClick={() => { setCustomMode(false); onChange(m) }}>{m}</button>
        ))}
        <button style={{ ...sx.toggle(customMode,'#7C3AED'), padding:'9px 14px', fontSize:12 }}
          onClick={() => { setCustomMode(true); onChange('') }}>✏ 직접입력</button>
      </div>
      {customMode && (
        <input style={sx.input} placeholder="담당자 이름 입력" value={isPreset?'':value} onChange={e=>onChange(e.target.value)} autoFocus/>
      )}
    </div>
  )
}
...
// ── 카테고리 추가 모달 ───────────────────────────────────────
function AddCategoryModal({ onAdd, onClose, existingColors }) {
  const [name, setName]   = useState('')
  const [icon, setIcon]   = useState('📦')
  const [color, setColor] = useState(COLOR_PALETTE.find(c=>!existingColors.includes(c))||COLOR_PALETTE[0])

  return (
    <div style={sx.overlay} onClick={onClose}>
      <div style={{ ...sx.modal, width:420 }} onClick={e=>e.stopPropagation()}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:22 }}>
          <span style={{ fontSize:16, fontWeight:800, color:C.text }}>카테고리 추가</span>
          <button onClick={onClose} style={{ background:'none', border:'none', color:C.muted, fontSize:22, cursor:'pointer' }}>×</button>
        </div>
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          <Field label="카테고리 이름 *">
            <input style={sx.input} value={name} onChange={e=>setName(e.target.value)} placeholder="예) 케이블, 공구, 조명장비" autoFocus/>
          </Field>
          <Field label="아이콘">
          ...
          <Field label="색상">
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              {COLOR_PALETTE.map(cl=>(
                <button key={cl} style={{ width:30, height:30, borderRadius:'50%', background:cl, border:cl===color?'3px solid #0F172A':'2px solid transparent', cursor:'pointer', transition:'border 0.15s' }}
                  onClick={()=>setColor(cl)}/>
              ))}
            </div>
          </Field>
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', gap:10, marginTop:22 }}>
          ...
          <button style={{ ...sx.btnPrimary, opacity:name.trim()?1:0.4 }} disabled={!name.trim()}
            onClick={()=>{ if(!name.trim()) return; onAdd({name:name.trim(),icon,color}); onClose() }}>추가</button>
        </div>
      </div>
    </div>
  )
...
export default function App() {
  ...
  const loadLogs = useCallback(async () => {
    const { data, error } = await supabase.from('logs').select('*').order('created_at', { ascending:false }).limit(300)
  ...
  useEffect(()=>{
    const init = async () => {
      setLoading(true)
      await Promise.all([loadItems(), loadLogs(), loadCategories()])
      setLoading(false)
    }
    init()

    // 실시간 동기화 — 다른 사람이 수정해도 자동 반영
    const ch1 = supabase.channel('items-rt').on('postgres_changes',{event:'*',schema:'public',table:'items'},loadItems).subscribe()
    const ch2 = supabase.channel('logs-rt').on('postgres_changes',{event:'*',schema:'public',table:'logs'},loadLogs).subscribe()
    const ch3 = supabase.channel('cats-rt').on('postgres_changes',{event:'*',schema:'public',table:'categories'},loadCategories).subscribe()

    return () => { ch1.unsubscribe(); ch2.unsubscribe(); ch3.unsubscribe() }
  }, [loadItems, loadLogs, loadCategories])

  // ── 로그 기록 ──
  const addLog = async (entry) => {
    await supabase.from('logs').insert([{ type:entry.type, manager:entry.manager, item_name:entry.item, detail:entry.detail }])
  }

  // ── 카테고리 CRUD ──
  const handleAddCategory = async (cat) => {
    const { error } = await supabase.from('categories').insert([{ name:cat.name, icon:cat.icon, color:cat.color, sort_order:categories.length }])
    if (error) { showToast('카테고리 추가 실패', 'error'); return }
  ...
  // ── 장비 CRUD ──
  const openAdd = () => {
    setForm({ category:categories[0]?.name||'', name:'', spec:'', serial:'', total:1, location:'1층', note:'', manager:'이건' })
  ...
  const saveItem = async () => {
    ...
    if (itemModal.mode === 'add') {
      const { error } = await supabase.from('items').insert([{
        category:form.category, name:form.name, spec:form.spec,
        serial:form.serial, total:form.total, location:form.location,
        note:form.note, manager:form.manager,
      }])
      if (error) { showToast('저장 실패: '+error.message, 'error'); setSaving(false); return }
      await addLog({ type:'추가', manager:form.manager||'미지정', item:form.name, detail:`${form.category} / ${form.location} / ${form.total}개` })
      showToast('장비가 추가됐어요!')
    } else {
      ...
      await addLog({ type:'수정', manager:form.manager||'미지정', item:form.name, detail:ch.join(', ')||'수정됨' })
      showToast('수정됐어요!')
    }
    await loadItems()
    setSaving(false)
  ...
  const downloadExcel = () => {
    ...
    if (logs.length>0) {
      ...
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(ld), '변경로그')
    }
    XLSX.writeFile(wb, `재고현황_${new Date().toLocaleDateString('ko-KR').replace(/\. /g,'-').replace('.','')}.xlsx`)
  }
  ...
  const logColor = { 추가:'#059669', 수정:'#0284C7', 삭제:'#DC2626', 업로드:'#7C3AED' }
    ...
    <div style={sx.app}>
      ...
      <div style={sx.sidebar}>
        <div style={sx.logoWrap}>
          ...
          <div style={sx.logoText}>GEAR VAULT</div>
        ...
        {tab==='inventory' && <>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8,flexWrap:'wrap',gap:10}}>
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
              ...
              <button style={sx.btnPrimary} onClick={openAdd}>+ 장비 추가</button>
            </div>
          </div>
                  ...
                  <tbody>
                    {fi.map(item=>{
                      ...
                      return <tr key={item.id} onMouseEnter={e=>e.currentTarget.style.background='#F8FAFC'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                        ...
                        <td style={sx.td}><span style={{fontWeight:800,fontSize:17,color:C.text}}>{item.total}</span></td>
        ...
        {tab==='categories' && <>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
            <div>
              ...
              <div style={sx.pageSub}>카테고리를 자유롭게 추가하거나 삭제할 수 있어요</div>
            </div>
            <button style={sx.btnPrimary} onClick={()=>setShowAddCat(true)}>+ 카테고리 추가</button>
          </div>
          ...
          <div style={{marginTop:14,padding:'12px 16px',background:'#FFF7ED',border:'1px solid #FED7AA',borderRadius:10,fontSize:12,color:'#92400E'}}>
            💡 장비가 등록된 카테고리는 삭제할 수 없어요.
          </div>
        </>}

        {/* ══ 변경 로그 ══════════════════════════════════ */}
        {tab==='logs' && <>
          <div style={sx.pageTitle}>변경 로그</div>
          <div style={sx.pageSub}>장비 추가·수정·삭제 전체 이력 ({logs.length}건)</div>
          {logs.length===0
            ? <div style={{...sx.card,textAlign:'center',padding:80,color:C.muted}}>아직 변경 이력이 없습니다.</div>
            : <div style={sx.card}>
                <table style={sx.table}>
                  <thead><tr>{['시간','유형','담당자','품목','변경 내용'].map(h=><th key={h} style={sx.th}>{h}</th>)}</tr></thead>
                  <tbody>
                    {logs.map(l=>(
      ...
      {itemModal && (
        <div style={sx.overlay} onClick={closeItemModal}>
          <div style={sx.modal} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:22}}>
              <span style={{fontSize:16,fontWeight:800,color:C.text}}>{itemModal.mode==='add'?'장비 추가':'장비 수정'}</span>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              ...
              <Field label="수량">
                <input style={{...sx.input,maxWidth:110}} type="number" min={1} value={form.total||1} onChange={e=>setForm({...form,total:parseInt(e.target.value)||1})}/>
              </Field>
              <Field label="위치">
                <div style={sx.trow}>
              ...
              <Field label="담당자">
                <ManagerPicker value={form.manager||''} onChange={v=>setForm({...form,manager:v})}/>
              </Field>
            ...
            <div style={{display:'flex',justifyContent:'flex-end',gap:10,marginTop:22}}>
              ...
              <button style={{...sx.btnPrimary,opacity:form.name&&!saving?1:0.5}} onClick={saveItem} disabled={!form.name||saving}>
                {saving?'저장 중...':'저장'}
              </button>
            </div>
          </div>
        ...
        <div style={sx.overlay} onClick={()=>setDeleteModal(null)}>
          <div style={{...sx.modal,width:420}} onClick={e=>e.stopPropagation()}>
            <div style={{textAlign:'center',marginBottom:22}}>
              ...
              <div style={{fontSize:13,color:C.muted}}>삭제 사유와 담당자를 선택해주세요.<br/>이력은 로그에 자동 저장됩니다.</div>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              <Field label="삭제 사유">
              ...
              <Field label="처리 담당자">
                <ManagerPicker value={deleteManager} onChange={setDeleteManager}/>
              </Field>
            </div>
            <div style={{display:'flex',gap:10,marginTop:22,justifyContent:'flex-end'}}>
              <button style={sx.btnGhost()} onClick={()=>setDeleteModal(null)}>취소</button>
              <button style={{...sx.btnDanger,padding:'9px 20px',fontSize:13,fontWeight:700}} onClick={confirmDelete} disabled={saving}>
                {saving?'처리 중...':'삭제 & 로그 저장'}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* ══ 카테고리 추가 모달 ══════════════════════════ */}
      {showAddCat && <AddCategoryModal onAdd={handleAddCategory} onClose={()=>setShowAddCat(false)} existingColors={categories.map(c=>c.color)}/>}
